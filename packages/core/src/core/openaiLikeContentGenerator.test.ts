...
describe('OpenAILikeContentGenerator', () => {
  let generator: OpenAILikeContentGenerator;
  ...
  beforeEach(() => {
    generator = new OpenAILikeContentGenerator(mockConfig);
    vi.resetAllMocks();
  });
  ...
  describe('generateContent', () => {
    ...
    it('should handle API errors', async () => {
      ...
      await expect(generator.generateContent(request)).rejects.toThrow(
        'OpenAI-like API error: 401 Unauthorized - Invalid API key'
      );
    });

    it('should handle baseURL with trailing slash', () => {
      const configWithSlash: OpenAILikeConfig = {
        apiKey: 'test-key',
        baseUrl: 'https://api.example.com/',
        modelName: 'test-model'
      };
      
      const gen = new OpenAILikeContentGenerator(configWithSlash);
  ...
  describe('embedContent', () => {
    it('should throw an error since OpenAI-like APIs typically do not support embeddings', async () => {
      const request = {
        model: 'test-model',
        contents: [
          {
